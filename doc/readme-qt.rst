Pawncoin-Qt: Qt4 GUI for Pawncoin
=================================
...
Alternatively, install `Qt Creator`_ and open the `pawncoin-qt.pro` file.

An executable named `pawncoin-qt` will be built.
...
A warning for people using the *static binary* version of Pawncoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Pawncoin is linked against libdb4.8 (see also `this Debian issue`_).
...
installed causes Pawncoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
